...
{
 Properties { _BaseColor("Color",Color)=(.4,1,.8,1) _HotColor("Core",Color)=(1,1,1,1) _Mode("Mode",Float)=0 _Glow("Glow",Float)=1 _Opacity("Opacity",Float)=1 _Phase("Phase",Float)=0 _Seed("Seed",Float)=0 _Inner("Inner clear radius",Float)=0 }
 ...
 float4 _BaseColor,_HotColor;float _Mode,_Glow,_Opacity,_Phase,_Seed,_Inner;
 CBUFFER_END
 ...
 else if(_Mode<1.5){float n=fbm(float3(p*7,_Seed+t*.2));float ridge=pow(1-abs(n*2-1),12);float cracks=pow(1-abs(noise3(float3(p*15,t*.12))*2-1),20);energy=(ridge*.7+cracks*.5)*smoothstep(1,.75,r);energy+=exp(-abs(r-.93)*100)*.6;if(_Inner>0&&r<_Inner)discard;energy*=smoothstep(_Inner-.08,_Inner+.06,r);core=energy*.3;}
