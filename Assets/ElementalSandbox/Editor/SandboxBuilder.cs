...
namespace ElementalSandbox.Editor
{
    ...
    public static class SandboxBuilder
    {
        ...
        {
            var settings=new SandboxSettings();if(settings.Abilities.Length!=10)throw new Exception("Expected ten abilities.");foreach(string shader in new[]{"Surface","Energy","Ground","Volume","Horizon","VenomSource","QuakeSource","SourceSmoke"}){var s=Shader.Find("Elemental/"+shader);if(s==null)throw new Exception("Missing shader "+shader);if(ShaderUtil.ShaderHasError(s))throw new Exception("Shader error "+shader);}
