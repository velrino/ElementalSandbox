...
namespace ElementalSandbox
{
    ...
    public sealed class ElementalApp : MonoBehaviour
    {
        ...
        public bool CastAt(int slot,Vector3 point,bool bypassCooldown=false)
        {
            ...
            if(IsRing(meta.id)){Vector3 f=actor.forward;f.y=0;dir=f.sqrMagnitude>.001f?f.normalized:Vector3.forward;point=actor.position+dir;}
        ...
        void UpdateAim()
        {
            Ray ray=View.ScreenPointToRay(pointer);pointerValid=new Plane(Vector3.up,Vector3.zero).Raycast(ray,out float enter)&&enter>0&&enter<500;if(pointerValid){Vector3 p=ray.GetPoint(enter);var id=Settings.Abilities[Selected].id;Vector3 delta=p-actor.position;float range=Mathf.Clamp(delta.magnitude,Settings.F(id+".minRange",0),Settings.F(id+".range",20));AimPoint=actor.position+delta.normalized*range;if(Armed&&!IsRing(id)&&Settings.B("character.turnToAim",true)&&delta.sqrMagnitude>.01f)actor.rotation=Quaternion.Slerp(actor.rotation,Quaternion.LookRotation(delta.normalized),1-Mathf.Exp(-10*Time.unscaledDeltaTime));}
            bool show=Armed&&pointerValid;var meta=Settings.Abilities[Selected];bool ring=IsRing(meta.id);zone.gameObject.SetActive(show&&meta.zone&&!ring);arrow.enabled=show&&!meta.zone&&!ring;aimMaterial.SetColor("_BaseColor",meta.Color);arrow.sharedMaterial.SetColor("_BaseColor",meta.Color);zone.position=AimPoint+Vector3.up*.05f;zone.rotation=Quaternion.Euler(90,0,0);zone.localScale=Vector3.one*Settings.F(meta.id+".zoneRadius",4)*2;
