    ...
    public static class SandboxSmokeTest
    {
        ...
        static void Update()
        {
            ...
            try{
                ...
                if(stage>=1&&stage<=10){int slot=stage-1;if(!waiting){app.Clear();app.ResetTargets();if(!app.CastAt(slot,new Vector3(0,0,6),true))throw new Exception("Cast rejected "+slot);waiting=true;deadline=EditorApplication.timeSinceStartup+(slot==3?.55:slot==9?2.8:1.8);return;}Capture(app,(slot+1).ToString("00")+"-"+app.Settings.Abilities[slot].id,1100,700);if(app.ActiveCount!=1)throw new Exception("Unexpected live effect count "+app.ActiveCount+" in "+slot);waiting=false;stage++;deadline=EditorApplication.timeSinceStartup+.1;return;}
                if(stage==11){app.Dummies[0].ResetBody();app.Dummies[0].Cut(Vector3.up);if(app.Dummies[0].SliceParts<2)throw new Exception("Body slicing failed");app.Clear();for(int i=0;i<5;i++)app.CastAt(i,new Vector3((i-2)*2,0,6),true);if(app.ActiveCount!=4)throw new Exception("Concurrent cast cap failed");app.TogglePause();stage++;deadline=EditorApplication.timeSinceStartup+.5;return;}
        ...
        static void Finish()
        {
            SessionState.SetBool("Elemental.Smoke",false);File.WriteAllText("Validation/SmokeTest.txt",errors.Count==0?"PASS: Ten abilities rendered; four-cast cap; body slicing; pause; landscape and portrait camera rendering.\n":"FAIL\n"+string.Join("\n",errors));bool exit=SessionState.GetBool("Elemental.ExitSmoke",false);Debug.Log("ELEMENTAL_SMOKE_"+(errors.Count==0?"PASS":"FAIL"));if(exit)EditorApplication.Exit(errors.Count==0?0:1);else EditorApplication.ExitPlaymode();
        }
    }
}
