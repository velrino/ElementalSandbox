...
namespace ElementalSandbox
{
    ...
    public sealed class ElementalApp : MonoBehaviour
    {
        ...
        readonly Vector4[] lights=new Vector4[4],lightColors=new Vector4[4],distortCenters=new Vector4[4],distortParams=new Vector4[4];
        ...
        void Awake()
        {
            ...
            CreateStage();CreateCharacter();CreateAim();ResetTargets();HUD=gameObject.AddComponent<SandboxHUD>();HUD.App=this;
        ...
        void CreateStage()
        {
            ...
            floorMaterial=new Material(Shader.Find("Elemental/Ground"));floorMaterial.SetTexture("_MainTex",Resources.Load<Texture2D>("Elemental/textures/cathedral/color"));floor.GetComponent<Renderer>().sharedMaterial=floorMaterial;
            ...
            volume=new GameObject("Post processing",typeof(Volume)).GetComponent<Volume>();volume.transform.SetParent(transform);volume.isGlobal=true;volume.profile=ScriptableObject.CreateInstance<VolumeProfile>();var bloom=volume.profile.Add<Bloom>();bloom.intensity.Override(.45f);bloom.threshold.Override(.85f);bloom.scatter.Override(.7f);var tone=volume.profile.Add<Tonemapping>();tone.mode.Override(TonemappingMode.ACES);var vignette=volume.profile.Add<Vignette>();vignette.intensity.Override(.2f);vignette.smoothness.Override(.65f);
        }
        ...
        public void Clear(){foreach(var e in effects)e.Retire();Armed=false;shake=0;Status="Effects cleared.";}
        ...
        void Update()
        {
            double start=Time.realtimeSinceStartupAsDouble;float real=Mathf.Min(Time.unscaledDeltaTime,.1f);float scale=Paused?0:Mathf.Clamp(Settings.F("global.timeScale",1),0,4);Time.timeScale=scale;float dt=real*scale;SimulationTime+=dt;Shader.SetGlobalFloat("_SandboxTime",SimulationTime);HandleInput();UpdateAim();UpdateEnvironment();
            int li=0;foreach(var e in effects){e.Tick(dt);if(e.Active&&li<4){lights[li]=new Vector4(e.Focus.x,1,e.Focus.z,e.F("lightRadius",12)*e.Envelope);Color c=Settings.Abilities[e.Slot].Color*e.Envelope*Settings.F("global.lightIntensity",1);lightColors[li]=new Vector4(c.r,c.g,c.b,1);Vector3 dc=e.Target+Vector3.up*(e.Id=="astral"?e.F("coreHeight",3.4f):1);distortCenters[li]=new Vector4(dc.x,dc.y,dc.z,e.Id=="astral"?e.F("coreRadius",1.05f)*2:e.Radius);distortParams[li]=new Vector4(e.Id=="astral"?1:0,e.Age,e.Envelope,Settings.F("global.distortion",1));li++;}}
            for(;li<4;li++){lights[li]=Vector4.zero;lightColors[li]=Vector4.zero;distortCenters[li]=Vector4.zero;distortParams[li]=Vector4.zero;}Shader.SetGlobalVectorArray("_ElementalLights",lights);Shader.SetGlobalVectorArray("_ElementalLightColors",lightColors);Shader.SetGlobalVectorArray("_DistortCenters",distortCenters);Shader.SetGlobalVectorArray("_DistortParams",distortParams);
        ...
        {
            View.backgroundColor=Settings.C("environment.backgroundColor");RenderSettings.fog=Settings.B("environment.fogEnabled",true);RenderSettings.fogMode=FogMode.Linear;RenderSettings.fogColor=Settings.C("environment.fogColor");RenderSettings.fogStartDistance=Settings.F("environment.fogNear",26);RenderSettings.fogEndDistance=Settings.F("environment.fogFar",135);RenderSettings.ambientMode=AmbientMode.Trilight;RenderSettings.ambientSkyColor=Settings.C("environment.hemiSkyColor")*Settings.F("environment.hemiIntensity",.36f);RenderSettings.ambientEquatorColor=Settings.C("environment.ambientColor")*Settings.F("environment.ambientIntensity",.14f);RenderSettings.ambientGroundColor=Settings.C("environment.hemiGroundColor")*.3f;
            sun.color=Settings.C("environment.sunColor");sun.intensity=Settings.F("environment.sunIntensity",2.6f);sun.transform.rotation=SourceLightRotation(Settings.F("environment.sunAzimuth",2.95f),Settings.F("environment.sunElevation",.6f));rim.color=Settings.C("environment.rimColor");rim.intensity=Settings.F("environment.rimIntensity",1.1f);rim.transform.rotation=SourceLightRotation(Settings.F("environment.rimAzimuth",5.45f),Settings.F("environment.rimElevation",.35f));floorMaterial.SetColor("_BaseColor",Settings.C("environment.floorColor"));floorMaterial.SetColor("_Tint",Settings.C("environment.floorTint"));floorMaterial.SetFloat("_Pool",Settings.F("environment.floorPool",.8f));floorMaterial.SetFloat("_Textured",Settings.B("environment.floorTexture")?1:0);
        }
        ...
        void OnDestroy(){Time.timeScale=1;foreach(var e in effects)e.Dispose();if(floorMaterial!=null)Destroy(floorMaterial);if(aimMaterial!=null)Destroy(aimMaterial);if(volume!=null&&volume.profile!=null)Destroy(volume.profile);Instance=null;}
    }
}
