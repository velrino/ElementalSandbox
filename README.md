...
The seven ground casts use a circle and Cyber Serpent a line arrow. **Venom Surge and Monolith Rift are guard casts**: no aim — click anywhere and the eruption rings the caster at `guardRadius` (venom 3.6 m, rift 4.5 m), the front sweeping the circle from where the body faces, the burst rising at the centre, and anything inside the ring struck as the front passes its bearing. **Arborist's Growth, Caustic Bloom and Cosmic Singularity are self casts** too: click anywhere and they rise round the caster — the vine cage closing over the body, the acid pool and its gas column ringing it with a clear centre of `acid.clearRadius` (1.6 m), the singularity opening overhead. Each first traces its circle: the seed runs round the caster over `sweepTime` (0.9 s) with a closing arc behind it, and the effect rises only once the ring has closed — the same beat the guard rings get from their eruption front. These are deliberate departures from the source, where all five are aimed. At most four casts remain active. Per-ability pools reuse visual objects. Targets use the original FBX with articulated Unity physics and respawn after dissolving.
